...
const INITIAL_AUDIO_ENABLED = true;
...
export default class RtcClient {
    ...
    async createOffer() {
        try {
            await this.rtcPeerConnection.createOffer();
        } catch (e) {
            console.error(e);
        }
    }
    ...
    setOntrack() {
        this.rtcPeerConnection.ontrack = (rtctTrackEvent) => {
            if (rtctTrackEvent.track.kind !== 'video') return;

            const remoteMediaStream = rtctTrackEvent.streams[0];
    ...
    async answer(sender, sessionDescription) {
        try {
            ...
            this.setOntrack();
            await this.setRemoteDescroption(sessionDescription);
    ...
    async setRemoteDescroption(sessionDescription) {
